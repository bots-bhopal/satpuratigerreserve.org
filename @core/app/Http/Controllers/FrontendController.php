...
class FrontendController extends Controller
{
    public function announcement_page()
    {
        ...
        return view('frontend.pages.aboard.announcement')->with(['announcements' => $announcements]);
    }

    public function dlink_page()
    {
        $dlists = DlCategory::with(['dlinks' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->get();
        return view('frontend.pages.dlink.dlinks')->with(['dlists' => $dlists]);
    }

    public function ntender_page()
    {
        $ntenders = NtCategory::with(['ntender' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->get();
        return view('frontend.pages.ntender.ntenders')->with(['ntenders' => $ntenders]);
    }
    ...
    public function strevent_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_events = Strevent::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('strevent_page_item'));
        return view('frontend.pages.strevents.event')->with([
            'all_events' => $all_events
        ]);
    }

    public function strevent_single_page($slug)
    {
        $strevent = Strevent::where('slug', $slug)->first();
        if (empty($strevent)) {
            abort(404);
        }

        return view('frontend.pages.strevents.event-single')->with([
            'strevent' => $strevent
        ]);
    }
    ...
    //     $default_lang = Language::where('default', 1)->first();
    //     $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
    //     $all_notices = NoticeOrder::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('notice_page_item'));
    //     return view('frontend.pages.notice.notices')->with([
    //         'all_notices' => $all_notices
    //     ]);
    // }

    // public function notice_single_page($slug)
    // {
    //     $notice = NoticeOrder::where('slug', $slug)->first();
    //     if (empty($notice)) {
    //         abort(404);
    //     }

    //     return view('frontend.pages.notice.notice-single')->with([
    //         'notice' => $notice
    //     ]);
    // }
    ...
    // public function downloadNotice($filename)
    // {
    //     $file_path = public_path('storage/notice_orders/') . $filename;
    //     if (file_exists($file_path)) {
    //         // Load PDF File
    //         return response()->file($file_path);
    //     } else {
    ...
    public function index()
    {
        ...
        // $all_documents = Document::orderBy('id', 'DESC')->get();
        // $all_notices = NoticeOrder::orderBy('updated_at', 'DESC')->get();
        $all_visitor_images = MonthPhoto::where('lang', $lang)->orderBy('updated_at', 'DESC')->latest()->first();
        // $all_gallery_images = ImageGallery::all();
        $all_gallery_images = ImageGallery::orderBy('id', 'desc')->take(10)->get();
        $announcements = Aboard::orderBy('updated_at', 'DESC')->get();
        // $all_visitors = Visitor::orderBy('updated_at', 'DESC')->first();
        // $all_links = Dlink::orderBy('updated_at', 'DESC')->get();
        $dlists = DlCategory::with(['dlinks' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->get();

        $ntlists = NtCategory::with(['ntender' => function ($q) {
            $q->orderBy('updated_at', 'DESC');
        }])->get();
        ...
        $blade_data = [
            ...
            'all_visitor_images' => $all_visitor_images,
        ];
