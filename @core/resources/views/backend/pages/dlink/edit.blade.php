    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <!-- form start -->
                <form action="{{ route('admin.dlink.update', $dlink->id) }}" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="card-body pb-3">
                                    <div class="form-group">
                                        ...
                                        <input type="title" class="form-control" id="title" name="title"
                                            value="{{ $dlink->title }}" placeholder="{{ __('Enter Title') }}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="categories" id="category"
                                            ...
                                            @foreach ($categories as $category)
                                                <option @if($dlink->dlcategory_id == $category->id) selected @endif value="{{$category->id}}">{{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    @if ($dlink->url)
                                        ...
                                        <div class="form-group website-url">
                                            ...
                                            <input type="url" class="form-control" id="customurl" name="customurl"
                                                value="{{ $dlink->url }}" placeholder="{{ __('https://www.example.com') }}"
                                                autocomplete="off">
                                        </div>
                                    @endif

                                    @if ($dlink->file_extension)
                                        @if ($dlink->file_extension == 'doc' || $dlink->file_extension == 'docx')
                                            <img src="{{ asset('assets/frontend/img/word.png') }}"
                                                ...
                                                alt="doc-image">
                                            <span
                                                style="font-weight: 600; position: relative; top: 4px;">{{ $dlink->original_filename }}</span>
                                        @endif
