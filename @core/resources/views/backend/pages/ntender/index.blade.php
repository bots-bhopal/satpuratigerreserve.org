            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12">
                        <div class="card mb-5">
                            <div class="card-body">
                                <h4 class="header-title">{{ __('Notices & Tenders') }}</h4>
                                <ul class="nav nav-tabs" id="myTab" role="tablist">
                                    @php $a=0; @endphp
                                    @foreach ($ntenders as $key => $ntender)
                                        <li class="nav-item">
                                            ...
                                            <tbody>
                                                @foreach ($ntenders as $key => $ntender)
                                                    <tr>
                                                        <td>{{ $ntender->id ?? '' }}</td>
                                                        <td>{{ $ntender->title ?? '' }}</td>
                                                        <td>{{ $ntender->original_filename ?? '' }}</td>
                                                        <td>{{ $ntender->url ?? '' }}</td>
                                                        <td class="text-center">
                                                            <a href="{{ route('admin.ntender.edit', $ntender->id) }}"
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-danger mt-1"
                                                                onclick="deleteTender({{ $ntender->id }})"
                                                                data-toggle="tooltip" data-placement="top" title="Delete"
                                                                style="font-size: 14px; font-weight: 500;"><i
                                                                    class="fa fa-trash-alt"></i> Delete</button>
                                                            <form id="delete-form-{{ $ntender->id }}"
                                                                action="{{ route('admin.ntender.destroy', $ntender->id) }}"
